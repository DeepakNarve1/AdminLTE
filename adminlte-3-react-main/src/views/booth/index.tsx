...
const Booth = () => {
  ...
  const [booths, setBooths] = useState<IBoothRow[]>([]);
  const [filteredBooths, setFilteredBooths] = useState<IBoothRow[]>([]);
  ...
  const fetchBooths = async () => {
    try {
      ...
      const res = await axios.get("http://localhost:5000/api/booths?limit=-1", {
      ...
      toast.error(err.response?.data?.message || "Failed to load booths");
    } finally {
      setLoading(false);
    }
  };
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(
        (b) =>
          ...
          b.block?.name?.toLowerCase().includes(term)
      );
    }
  ...
  const paginatedBooths = useMemo(() => {
    if (entriesPerPage === -1) return filteredBooths;
    const start = (currentPage - 1) * entriesPerPage;
    return filteredBooths.slice(start, start + entriesPerPage);
  }, [filteredBooths, currentPage, entriesPerPage]);

  const totalPages = Math.ceil(
    filteredBooths.length /
      (entriesPerPage === -1 ? filteredBooths.length || 1 : entriesPerPage)
  );
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`http://localhost:5000/api/booths/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      toast.success("Booth deleted successfully");
      fetchBooths();
    } catch (err: any) {
      toast.error(err.response?.data?.message || "Could not delete booth");
    }
  };
              ...
              <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-center justify-between">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {hasPermission("create_booths") && (
                    ...
                    >
                      <Plus className="w-5 h-5 mr-2" /> Add New Booth
                    </Button>
                  )}
                </div>
              </div>
            ...
            <div className="border-t border-gray-200 p-6">
              <div className="flex items-center justify-between">
                <p className="text-sm text-gray-600">
                  ...
                  of {filteredBooths.length} booths
                </p>
