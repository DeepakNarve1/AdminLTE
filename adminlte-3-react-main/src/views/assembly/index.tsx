...
const Assembly = () => {
  ...
  const [assemblies, setAssemblies] = useState<IAssemblyRow[]>([]);
  const [filteredAssemblies, setFilteredAssemblies] = useState<IAssemblyRow[]>(
    []
  );
  ...
  const fetchAssemblies = async () => {
    try {
      ...
      toast.error(err.response?.data?.message || "Failed to load assemblies");
    } finally {
      setLoading(false);
    }
  };
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(
        (a) =>
          ...
          a.parliament?.name?.toLowerCase().includes(term)
      );
    }
  ...
  const paginatedAssemblies = useMemo(() => {
    if (entriesPerPage === -1) return filteredAssemblies;
    const start = (currentPage - 1) * entriesPerPage;
    return filteredAssemblies.slice(start, start + entriesPerPage);
  }, [filteredAssemblies, currentPage, entriesPerPage]);

  const totalPages = Math.ceil(
    filteredAssemblies.length /
      (entriesPerPage === -1 ? filteredAssemblies.length || 1 : entriesPerPage)
  );
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`http://localhost:5000/api/assemblies/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      toast.success("Assembly deleted successfully");
      fetchAssemblies();
    } catch (err: any) {
      toast.error(err.response?.data?.message || "Could not delete assembly");
    }
  };
              ...
              <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-center justify-between">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {hasPermission("create_assemblies") && (
                    ...
                    >
                      <Plus className="w-5 h-5 mr-2" /> Add New Assembly
                    </Button>
                  )}
                </div>
              </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-gray-50">
                    ...
                    <TableHead className="font-semibold">Parliament</TableHead>
                ...
                <TableBody>
                  {loading ? (
                    ...
                    paginatedAssemblies.map((assembly, index) => (
                      <TableRow
                        key={assembly._id}
            ...
            <div className="border-t border-gray-200 p-6">
              <div className="flex items-center justify-between">
                <p className="text-sm text-gray-600">
                  ...
                  of {filteredAssemblies.length} assemblies
                </p>
